...
export default function ToolMenu({ active, threadView, onSelect }) {
  ...
  return (
    <nav className="z-50 w-full rounded-[24px] border border-white/[0.08] bg-bg/82 p-1.5 shadow-[0_18px_60px_rgba(0,0,0,0.55)] backdrop-blur-2xl sm:rounded-[28px] sm:p-2">
        ...
        {items.map(item => {
          ...
          return (
            <button
              ...
              className={`flex min-w-0 flex-1 items-center gap-2 rounded-[18px] border px-2 py-2 transition-all duration-300 sm:gap-3 sm:rounded-[22px] sm:px-4 sm:py-3 ${
                isActive
            ...
            >
              <div
                className={`flex h-8 w-8 shrink-0 items-center justify-center rounded-xl border sm:h-11 sm:w-11 sm:rounded-2xl ${
                  isActive
              ...
              <div className="min-w-0 text-left">
                <div className="text-[0.72rem] font-bold tracking-tight sm:text-sm">{item.label}</div>
                <div
                  className={`text-[0.5rem] uppercase tracking-[0.18em] sm:text-[0.65rem] sm:tracking-[0.28em] ${
